...
export default async function TempsDeJeuPage({ searchParams }: { searchParams: Promise<{ team?: string; periode?: string }> }) {
  ...
  const activeCategory = activeGroup?.categories.find((c) => categoryOptions.includes(c));
  const category = categoryOptions.includes(sp.team ?? "") ? sp.team! : (activeCategory ?? availableCategories[0]);
