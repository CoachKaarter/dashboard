/**
 * Seed script — realistic French U12/U13 football club demo data.
 * Deterministic (fixed RNG seed) so re-running `prisma db seed` after a
 * `migrate reset` reproduces the same dataset.
 */
...
// ---------- deterministic RNG (same LCG shape as the design prototype) ----------
let seed = 20262027;
function rnd() {
...
async function main() {
  ...
  console.log(`Seeded ${players.length} players across ${TEAM_DEFS.length} teams.`);
  console.log(`Demo accounts: ${STAFF_DEFS.map((s) => s.username).join(", ")} — password "${DEMO_PASSWORD}"`);
}
